...
namespace rd {
    ...
    double TimeFromTimingKind(const TimingData &Timing,
                              ...
                              const double Time,
                              Difficulty::ETimingType TimingType,
                              double Offset) {
        if (TimingType == Difficulty::BT_BEAT) // Time is in Beats
        {
            return TimeAtBeat(Timing, Offset, Time) + StopTimeAtBeat(StopsTiming, Time);
        } else if (TimingType == Difficulty::BT_MS ||
                   TimingType == Difficulty::BT_BEATSPACE) // Time is in MS
        {
            return Time + Offset;
        }
    ...
    TimingData GetVSpeeds(TimingData &BPS, double ConstantUserSpeed) {
        ...
        for (auto Section : BPS) {
            ...
            if (Section.Value) {
                ...
                VerticalSpeed = 1 /* UNITS_PER_MEASURE */ / (spb * 4);
            } else
                VerticalSpeed = 0;
        ...
        if (!VerticalSpeeds.empty() && VerticalSpeeds[0].Value == 0) {
    ...
    PlayerChartState PlayerChartState::FromDifficulty(Difficulty *diff, double ConstantUserSpeed) {
        ...
        if (!ConstantUserSpeed) {
            ...
            out.interpolatedSpeedMultipliers = data->InterpoloatedSpeedMultipliers;
        }
        ...
        for (auto S : out.interpolatedSpeedMultipliers) if (S.Value < 0) out.HasNegativeScroll = true;
    ...
    double PlayerChartState::GetSpeedMultiplierAt(double Time) const {
        ...
        auto speedIter = lower_bound(
                interpolatedSpeedMultipliers.begin(),
                interpolatedSpeedMultipliers.end(),
                CurrentTime
        );
        ...
        if (speedIter != interpolatedSpeedMultipliers.begin())
            speedIter--;
        ...
        */
        ...
        {
            if (speedIter != interpolatedSpeedMultipliers.end())
