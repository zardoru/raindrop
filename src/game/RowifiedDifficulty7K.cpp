...
RowifiedDifficulty::RowifiedDifficulty(Difficulty *Source, bool Quantize, bool CalculateAll)
    ...
    BPS = rd::PlayerChartState::FromDifficulty(Source).BPS;
...
bool RowifiedDifficulty::IsQuantized() const {
    return Quantizing;
}
        ...
        for (int K = 0; K < Parent->Channels; K++) {
            for (auto N : M.Notes[K]) {
                ...
                if (N.EndTime == 0) { // Non-hold. Emit channels 11-...
                    ...
                    auto Snd = N.Sound ? N.Sound : 1;
                    Measures[MeasureForEvent].LNObjects[K].push_back(
                            {FractionForMeasure(MeasureForEvent, StartBeat), Snd});
                    Measures[MeasureForEventEnd].LNObjects[K].push_back(
                            {FractionForMeasure(MeasureForEventEnd, EndBeat), Snd});
                }
            }
        }
    }
