...
class OsumaniaLoader
{
    ...
    std::unique_ptr<Difficulty> Diff;
	...
	{
		TimingData BPS = rd::GetBPSData(Diff.get());
...
public:
	...
	*/
	std::string GetSampleFilename(SplitResult &split_line, int NoteType, int Hitsound, double Time)
	{
		...
		if (CustomSample)
			custom_sample = IntToStr(CustomSample);
	...
	void LoadFromFile(const std::filesystem::path& path)
    {
		...
		Diff = std::make_unique<Difficulty>();
		...
		*/
		...
		try
		{
			while (filein)
			{
				...
				if (ReadingMode != ReadingModeOld || ReadingMode == RNotKnown) // Skip this line since it changed modes, or it's not a valid section yet
				{
					...
					if (ReadingModeOld == REvents)
					    Diff->Data->osbSprites = EventsContent.str();
			...
			if (notecount)
			{
				...
				osu_sng->Difficulties.push_back(std::move(Diff));
			}
		}
...
void NoteLoaderOM::LoadObjectsFromFile(const std::filesystem::path& filename, Song *Out)
{
