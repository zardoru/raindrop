...
namespace NoteLoaderBMS{
	...
	class BMSLoader
	{
		...
		void ForAllEventsInChannel(const std::function<void(BMSEvent,int)> &fn, int Channel)
		{
			for (auto & Measure : Measures)
				for (auto ev : Measure.second.Events[Channel])
					fn(ev, Measure.first);
		}
		...
		void CalculateBMP(std::vector<AutoplayBMP> &BMPEvents, int Channel)
		{
			ForAllEventsInChannel([&](BMSEvent ev, int Measure) {
						BMPEvents.emplace_back(
                            TimeForObj(Measure, ev.Fraction),
                            ev.Event
						);
			}, Channel);
		}
		...
		void CalculateMeasure(BMSMeasureList::iterator &bms_measure)
		{
			...
			else
			{
				...
				CalculateMeasureSide(
				        bms_measure,
				        5,
				        startChannelP2 + 1, startChannelLNP2 + 1,
				        startChannelMinesP2 + 1, startChannelInvisibleP2 + 1,
				        Msr
                );
			}
			...
			if (!bms_measure->second.Events[CHANNEL_BGM].empty()) // There are some BGM events?
			{
				for (auto ev : bms_measure->second.Events[CHANNEL_BGM])
				{
					...
					UsedSounds[ev.Event] = true;
					Chart->Data->BGMEvents.emplace_back(Time, ev.Event);
				}
	...
	public:
		...
		BMSLoader(rd::Song* song, std::unique_ptr<rd::Difficulty>& diff, bool ispms)
		{
		...
		void ParseEvents(const int Measure, const int BmsChannel, const std::string &Command)
		{
			...
			if (BmsChannel != CHANNEL_METER)
			{
				...
				{
				    std::string CharEvent = Command.substr(i * 2, 2);
					int Event = b36toi(CharEvent.c_str());
					double Fraction = double(i) / CommandLength;

					if (Event == 0) // Nothing to see here?
						continue;

					Measures[Measure].Events[BmsChannel].push_back({Event, Fraction});
				}
			}
		...
		bool InterpStatement(const std::string& Command, const std::string& Contents)
		{
			...
			do {
				...
				{
					RandomStack[CurrentNestedLevel] = std::stoi(Contents);
				}
				else if (Command == "#random")
				{
					...
					int Limit = std::stoi(Contents);

					assert(CurrentNestedLevel < 16);
					//assert(Limit > 1);

					std::uniform_int_distribution<int> range(1, Limit);
					RandomStack[CurrentNestedLevel] = range(randgen);
		...
		void CompileBMS()
		{
			...
			for (auto & Sound : Sounds)
				if (UsedSounds.find(Sound.first) != UsedSounds.end() && UsedSounds[Sound.first]) // This sound is used.
					Chart->Data->SoundList[Sound.first] = Sound.second;
	...
	void LoadObjectsFromFile(const std::filesystem::path& filename, Song *Out)
	{
        ...
        auto Diff = std::make_unique<Difficulty>();
        bool IsPMS = false;
		...
		auto fn = Conversion::ToU8(filename.wstring());
        if (!filein.is_open())
...
#define OnCommandSub(x) if(command.substr(0, strlen(#x)) == #x)
			...
			auto parse_line = [&]() {
				...
				std::string tmp;
				if (!IsU8)
					CommandContents = Conversion::SJIStoU8(CommandContents);
				...
				try {
					...
					if (IsU8) {
						IsU8 = false;
						tmp = Conversion::SJIStoU8(CommandContents);
					}
				}
				CommandContents = tmp;
			};
            ...
            if (Info->InterpStatement(command, CommandContents))
            {
                ...
                OnCommand(#offset)
                {
                    Diff->Offset = latof(CommandContents);
                }
				...
				OnCommand(#defexrank)
				{
					Info->SetDefexRank(latof(CommandContents));
				}
                ...
                OnCommand(#difficulty)
                {
                    ...
                    {
                        int Kind = std::stoi(CommandContents);
                ...
                OnCommandSub(#bpm)
                {
                    ...
                    int Index = b36toi(IndexStr.c_str());
                    Info->SetBPM(Index, latof(CommandContents));
                }
                ...
                OnCommandSub(#stop)
                {
                    ...
                    int Index = b36toi(IndexStr.c_str());
                    Info->SetStop(Index, latof(CommandContents));
                }
				...
				OnCommandSub(#scroll)
                {
                    ...
                    int Index = b36toi(IndexStr.c_str());
                    Info->SetScroll(Index, latof(CommandContents));
                }
                ...
                if (regex_match(MainCommand, sm, RegexDataDeclaration)) // We've got work to do.
                {
        ...
        std::string sf = Conversion::ToU8(std::filesystem::path(filename).filename().wstring());
