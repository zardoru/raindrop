...
public:
	...
	void ReadPackages(OjnHeader &Head, int difficulty_index, std::fstream &ojnfile)
	{
		...
		{
			OjnPackageHeader PackageHeader = {0};
			...
			for (size_t event_index = 0U; event_index < PackageHeader.events; ++event_index)
			{
				...
				OjnPackage package = {0};
				OjnExpandedPackage o2evt = {0};
		...
		for (auto &Measure : Measures)
		{
			...
			for (auto & Event : Measure.Events)
			{
				if (Event.Channel < AUTOPLAY_CHANNEL)
				{
					...
					{
						if (prevIter[Event.Channel].noteKind == 2 &&
							(Event.noteKind == 0 || Event.noteKind == 2)) // This note or hold head is in between holds
						{
						...
						if (prevIter[Event.Channel].noteKind != 2 && // Hold tail without ongoing hold
							Event.noteKind == 3)
						{
							Event.Channel = AUTOPLAY_CHANNEL;
							Event.noteKind = 0;
	...
	void CopyOJNTimingData(Difficulty *Out)
	{
		...
		for (const auto& Measure : Measures)
		{
			...
			Out->Data->Measures.push_back(rd::Measure());
			...
			for (auto Evt : Measure.Events)
			{
              ...
              because o2jam's measure fractions don't scale the notes inside. */
				...
				Out->Timing.push_back({Beat, Evt.fValue});
			}
	...
	void OutputAllOJNEventsToDifficulty(rd::Difficulty *Out)
	{
...
void NoteLoaderOJN::LoadObjectsFromFile(const std::filesystem::path& filename, rd::Song *Out)
{
	std::fstream filein(filename, std::ios::in | std::ios::binary);

    OjnHeader Head = {0};
	auto ufn = Conversion::ToU8(filename.wstring());
    ...
    */
    ...
    for (auto i = 0; i < 3; i++)
    {
        ...
        Diff->Data->TimingInfo = std::make_unique<rd::O2JamChartInfo>();
        auto TInfo = static_cast<rd::O2JamChartInfo*>(Diff->Data->TimingInfo.get());

        switch (i)
        {
        case 0:
            TInfo->Difficulty = rd::O2JamChartInfo::O2_EX;
            break;
        case 1:
            TInfo->Difficulty = rd::O2JamChartInfo::O2_NX;
            break;
        case 2:
        default:
            TInfo->Difficulty = rd::O2JamChartInfo::O2_HX;
            break;
        }
        ...
        Diff->Author = Noter;
        Diff->Data->StageFile = Conversion::ToU8(filename.filename().wstring());

        Info.S = Out;
        filein.seekg(Head.note_offset[i]);

        // O2Jam files use Beat-Based notation.
        Diff->BPMType = rd::Difficulty::BT_BEAT;
