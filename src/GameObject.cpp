...
namespace Game {
	namespace dotcur {
		...
		Judgment GameObject::Hit(double Time, Vec2 mpos, bool KeyDown, bool Autoplay, int32_t Key)
		{
			...
			if (dabs < CircleSize && fadeout_time == 0)
			{
				...
				if (SpareTime < LeniencyHitTime) // within leniency?
				{
					...
					if (SpareTime < LE_GREAT)
						RVal = J_GREAT;

					if (SpareTime < LE_PERFECT)
						RVal = J_PERFECT;

					if (SpareTime < LE_EXCELLENT)
						RVal = J_EXCELLENT;
		...
		void GameObject::Assign(double Duration, uint32_t _Measure, double _MeasureFraction)
		{
