...
AudioDataSource* SourceFromExt(Directory Filename)
{
    ...
    if (Ret) {
        if (!Ret->Open(Filename.c_path())) {
            // Chances are if we get to this point, we were provided the wrong extension.
            char numOfAttempts = 0;
            char maxAttempts = 1;
            if (!(strstr(xt, "wav") || strstr(xt, "flac"))) {
                maxAttempts++;
            }
            ...
            while (numOfAttempts < maxAttempts) {
                ...
                if (strstr(xt, "wav") || strstr(xt, "flac")) {
                    ...
                    switch (numOfAttempts) {
                        case 0:
                            Ret = new AudioSourceOGG();
                            dir.ReplaceExtension("ogg");
                            break;
#ifdef MP3_ENABLED
                        case 1:
                            Ret = new AudioSourceMP3();
                            dir.ReplaceExtension("mp3");
                            break;
#endif
                        default:
                            break;
                ...
                if (strstr(xt, "mp3")) {
                    ...
                    switch (numOfAttempts) {
                        ...
                        case 1:
                            Ret = new AudioSourceSFM();
                            dir.ReplaceExtension("wav");
                            break;
                            
                        case 2:
                            Ret = new AudioSourceSFM();
                            dir.ReplaceExtension("flac");
                            break;
