...
if Upscroll == 0 then
	JudY = ScreenHeight - GearHeight - 150
else
	JudY = GearHeight + 150
end
...
function Lifebar.Run(Delta)
	...
	Obj.SetPosition( Lifebar.CurrentPosition.x - 13, NewYFixed );
	...
	Obj.SetPosition( Lifebar.CurrentPosition.x - 13, NewY );
...
function Judgment.Run(Delta)
	if Active ~= 0 then
		...
		if Judgment.Value ~= 1 and Judgment.Value ~= 6 then -- not a "flawless"
			Obj.SetAlpha(AlphaRatio)
